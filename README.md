...
### 1. Backend báo không kết nối được database
...
Nếu container mới tạo lần đầu, đợi thêm 10-30 giây rồi chạy lại backend.

### 2. Port 3307 bị trùng

Nếu máy đã có service khác dùng port `3307`, sửa port trong `docker-compose.yml`:
...
Nếu đổi backend port, frontend cũng cần đổi API URL bằng file `frontend/.env`:
...
Kiểm tra frontend đang gọi đúng API URL. Mặc định trong `frontend/src/api/todoApi.ts` là:

```txt
http://localhost:5000/api
```

### 5. Lệnh `docker compose` không chạy

Thử dùng lệnh legacy:

```bash
docker-compose up -d
```

Hoặc kiểm tra Docker Compose:
...
### 6. `npm audit` báo moderate vulnerabilities ở backend

Backend có thể báo cảnh báo từ dependency `sequelize -> uuid`.
...
Vì lệnh này có thể cài version breaking change. Cảnh báo này không làm project bị lỗi khi chạy local.

## Trạng Thái Kiểm Thử Hiện Tại

Đã kiểm tra:

- Docker MySQL chạy OK.
- Backend kết nối database OK.
- Backend sync table OK.
- `GET /` trả `200`.
- `GET /api/todos` trả `200`.
